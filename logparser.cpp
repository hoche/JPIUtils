...
static void read_file(const char* szFilename)
{
	...
	int fd = open(szFilename, _O_BINARY | O_RDONLY );
	if(fd == -1)
	...
	if((s_nFileBytes = read(fd,s_pFileBytes,filestats.st_size)) <= 0)
...
static void setdir(const char* basenam,char* outname,size_t outsize)
{
	...
	_splitpath_s(s_szCurrFile,drive,_MAX_DRIVE,dir,_MAX_DIR,NULL,0,NULL,0);
	_splitpath(basenam,NULL,0,NULL,0,name,_MAX_FNAME,ext,_MAX_EXT);
...
static void write_file(const char* szFilename,const void* bytes,size_t nbytes)
{
	...
	int fd = open(szFilename, _O_BINARY | O_CREAT | O_WRONLY | O_TRUNC, _S_IREAD | _S_IWRITE);
	if(fd == -1)
